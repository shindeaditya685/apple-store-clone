import React from "react";

export default function MacDropdown() {
  return (
    <div className=" w-[100vw] ">
      {/* bg-[#f5f5f7] */}
      <div className="bg-[#f5f5f7] shadow-xl ">
        <div className=" flex w-full lg:px-[13.8rem] md:px-[0.8rem] md:pt-[1rem] lg:pt-[2rem] pb-[5rem]  gap-[4rem]">
          <div className="flex flex-col gap-2 justify-center items-start ">
            <h1 className=" text-gray-400 text-[1rem]">Explore Mac</h1>
            <ul className=" text-3xl font-bold flex flex-col gap-4 tracking-wide">
              <li>Explore All Mac</li>
              <li>MacBook Air</li>
              <li>MacBook Pro</li>
              <li>iMac</li>
              <li>Mac Studio</li>
              <li>Mac Pro</li>
              <li>Displays</li>
            </ul>
            <div className=" pt-2">
              <h1 className=" text-gray-400 text-[1rem]">Compare Mac</h1>
              <ul>
                <li className=" text-gray-400 pt-2">Mac Does That</li>
              </ul>
            </div>
          </div>
          <div className="flex flex-col gap-2   ">
            <h1 className=" text-gray-400 text-[1rem]">Shop Mac</h1>
            <ul className=" flex flex-col gap-4">
              <li>Shop Mac</li>
              <li>Mac Accessories</li>
              <li>Ways to Buy</li>
            </ul>
          </div>
          <div className="flex flex-col gap-2   ">
            <h1 className=" text-gray-400 text-[1rem]">More from Mac</h1>
            <ul className=" flex flex-col gap-4">
              <li>Mac Support</li>
              <li>AppleCare+ for Mac</li>
              <li>macOS Sonoma</li>
              <li>Apps by Apple</li>
              <li>Continuity</li>
              <li>iClouds+</li>
              <li>Mac for Business</li>
              <li>Education</li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  );
}
